...
!  call get_state('z',g,z)
 ...
 DO ns = 1, mstage
    ...
    elseif(spherical==1) then
      pdx(:) = x(:) + a_rk(ns)*deltaT*chix(:,ns)/(tpi*COSD(pdy(:))+eps)
      pdy(:) = y(:) + a_rk(ns)*deltaT*chiy(:,ns)/tpi
 ...
 do ns=1,mstage
     if(spherical==0) then
      pdxt(:) = pdxt(:) + b_rk(ns)*deltaT*chix(:,ns)
      pdyt(:) = pdyt(:) + b_rk(ns)*deltaT*chiy(:,ns)
    elseif(spherical==1) then
      pdxt(:) = pdxt(:) + b_rk(ns)*deltaT*chix(:,ns)/(tpi*COSD(pdyt(:))+eps)
      pdyt(:) = pdyt(:) + b_rk(ns)*deltaT*chiy(:,ns)/tpi
